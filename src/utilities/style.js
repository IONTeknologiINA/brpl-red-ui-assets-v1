    ...
    multiselect: {
        component: function (fetched, tag) {
            return {
                ...
                dropdown: !fetched ? 'hidden' : `max-h-72 absolute -left-px -right-px -bottom-1 transform translate-y-full border-2 pb-1 border-${dropdown} -mt-px overflow-y-auto z-30 bg-white flex flex-col rounded${round}`,
                dropdownTop: `-translate-y-full top-px bottom-auto rounded-${round}`,
                dropdownHidden: 'hidden',
                caret: `px-3 bg-multiselect-caret bg-center bg-no-repeat w-2.5 h-4 py-px box-content mr-3.5 relative opacity-40 flex-shrink-0 flex-grow-0 transition-transform transform pointer-events-none rounded${round} `,
